...
serviceController.showAddForm = async (req, res, next) => {
  try {
    ...
    const vehicle = await inventoryModel.getVehicleById(inv_id); // change method name if needed
    if (!vehicle) return res.status(404).render('errors/404', { message: 'Vehicle not found' });

    res.render('service/add-service', { vehicle, errors: null, formData: {} });
  } catch (err) {
    next(err);
  }
};
...
serviceController.validate = [
  body('service_date').isISO8601().withMessage('Service date is required and must be a date'),
...
serviceController.addService = async (req, res, next) => {
  try {
    ...
    if (!errors.isEmpty()) {
      ...
      return res.status(400).render('service/add-service', { vehicle, errors: errors.array(), formData });
    }

    const insertId = await ServiceModel.addService(formData);

    // Redirect to the service list after successful insert
    req.flash('success', 'Service record saved.'); // if using connect-flash
...
serviceController.listByVehicle = async (req, res, next) => {
  try {
    ...
    res.render('service/service-list', { vehicle, records });
  } catch (err) {
    next(err);
  }
};
